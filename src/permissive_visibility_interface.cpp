...
bool PermissiveVisibilityInterfaceGDExt::can_tile_see(Vector2i origin, Vector2i target) {
	...
	bool *visibilityFromOrigin = data->visibilityMap[data->to_map_index_v(origin)];

	if (visibilityFromOrigin == nullptr) {
		visibilityFromOrigin = _calculate_sightlines_from_tile(origin.x, origin.y);
		data->visibilityMap[data->to_map_index_v(origin)] = visibilityFromOrigin;
	}

	return visibilityFromOrigin[data->to_map_index_v(target)];
}
...
bool *PermissiveVisibilityInterfaceGDExt::_calculate_sightlines_from_tile(int x, int y) {
	...
	if (data->visibilityMap[data->to_map_index_v(data->currentOrigin)] != nullptr) {
		delete[] data->visibilityMap[data->to_map_index_v(data->currentOrigin)];
	}
	bool *los_map = new bool[data->width * data->height];
	memset(los_map, false, data->width * data->height);
	data->visibilityMap[data->to_map_index_v(data->currentOrigin)] = los_map;
...
PermissiveVisibilityInterfaceGDExt::PermissiveVisibilityInterfaceGDExt() {
	data = memnew(PermissiveVisibilityDataGDExt);
}
